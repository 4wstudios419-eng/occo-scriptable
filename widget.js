...
if (!config.runsInWidget && (qp.mode === "cad" || qp.action === "cad")) {
  await cadQuickStart(); Script.complete(); return;
}
...
try {
  ...
  try {
    ...
    const badgeOuter = w.addStack(); badgeOuter.layoutHorizontally(); badgeOuter.addSpacer();
    const badgeImg = badgeOuter.addImage(badge); badgeImg.imageSize = new Size(58, 58); badgeOuter.addSpacer();
    w.addSpacer(3);
  } catch {}
  addHeader("OTTAWA COUNTY"); addSubheader("CORONER'S OFFICE"); w.addSpacer(6);
  addBig("Next Case", data?.nextCaseNumber); w.addSpacer(5);
  addSection("On Call"); addRow("Investigator", data?.onCallInvestigator);
  if (data?.currentInvestigatorPhone) addRow("Phone", data.currentInvestigatorPhone);
  w.addSpacer(5); addSection("Funeral Home"); addRow("", data?.funeralHomeOfMonth);
  const wx = await getWeatherSafe();
  if (wx) { w.addSpacer(5); addWeather(wx.temp, wx.feels, wx.label); }
  w.addSpacer(); addFooter("Tap for CAD Quick Start • Updated " + formatTime(data?.updatedAt));
} catch (err) {
  addHeader("OCCO"); w.addSpacer(8); let error = w.addText("Dashboard unavailable"); error.textColor = Color.red(); error.font = Font.systemFont(10);
}
...
async function cadQuickStart(){
  try {
    ...
    third.title = "Reported Circumstances";
    third.message = "ATLAS will default this case to Pending Review, Undetermined / Pending, reason: Pending Investigation.";
    third.addTextField("Brief circumstances", "");
    third.addAction("Continue"); third.addCancelAction("Cancel");
    if (await third.presentAlert() < 0) return;
    const circumstances = third.textFieldValue(0).trim();

    const record = new Alert();
    record.title = "Record Type";
    record.message = "Use Test / Training while we are validating the widget.";
    record.addAction("Official"); record.addAction("Test / Training"); record.addCancelAction("Cancel");
    ...
    const caseRow = await createNumberedCase(session, profile.user_id, "investigative", recordClass);
    await saveDecedent(session, caseRow.id, decedent, profile.user_id);
    ...
    const ingest = await postJson(CAD_INGEST_URL, payload, session.access_token);
    const done = new Alert();
    done.title = `${caseRow.case_number} Created`;
    done.message = ingest?.status === "needs_review"
      ? "Case created. ATLAS received the CAD intake, but one or more fields need review."
      : "Case created. Dispatch is populated and remains pending investigation until you update it in ATLAS.";
...
async function createNumberedCase(session, leadId, category, recordClass){
  const result = await postJson(`${SUPABASE_URL}/rest/v1/rpc/create_numbered_case`, {
    p_case_category:category,
    p_record_class:recordClass,
    p_lead_investigator_id:leadId,
    p_record_class_reason:recordClass === "test_training" ? "CAD widget validation" : null
  }, session.access_token);
  return Array.isArray(result) ? result[0] : result;
}

async function saveDecedent(session, caseId, decedent, userId){
  const req = new Request(`${SUPABASE_URL}/rest/v1/case_decedent?on_conflict=case_id`);
  req.method="POST";
  ...
  req.body=JSON.stringify([{case_id:caseId,decedent_name:decedent,updated_by:userId}]);
  await req.loadString();
  if(req.response.statusCode>=300) throw new Error(`Unable to save decedent identifier (${req.response.statusCode}).`);
}

async function postJson(url, body, token){
  const req = new Request(url); req.method="POST";
  req.headers={apikey:SUPABASE_KEY,"Content-Type":"application/json"}; if(token) req.headers.Authorization=`Bearer ${token}`;
...
async function getJson(url, token){
  ...
  const text=await req.loadString(); let d=null; try{d=text?JSON.parse(text):null;}catch{}
  if(req.response.statusCode>=300) throw new Error(d?.message||text||`Request failed (${req.response.statusCode})`); return d;
}
